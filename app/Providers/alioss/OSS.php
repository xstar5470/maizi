...
class OSS
{
    ...
    public function __construct($isInternal = false)
    {
        ...
        $this->ossClient = AliyunOSS::boot(
            config('alioss.ossCity'),config('alioss.ossNetWorkType'),
            $isInternal,
    ...
    public static function upload($ossKey, $filePath,$optional = [])
    {

        $oss = new OSS(false); // 上传文件使用内网，免流量费
        $oss->ossClient->setBucket(config('alioss.BucketName'));

        $res = $oss->ossClient->uploadFile($ossKey, $filePath,$optional);
        return $res;
    }
